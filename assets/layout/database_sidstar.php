		...
		// Loops through the Query, filters out the correct Procedures.
 		for($i=0; count($sidData) > $i; $i++) {
			...
			if (empty($sid[$sidData[$i]["sidname"]])){
				$sid[$sidData[$i]["sidname"]] = array(
					'icao' => $sidData[$i]["icao"],
					'aerodome' => array(
							'lat' => $coordinates[0]["lon"],
							'lon' => $coordinates[0]["lat"]
					),
					'runways'=> $sidData[$i]["runways"],
					'waypoints' => array(
						array(
							'wypname'=> $sidData[$i]["wypname"],
							'lat' => $sidData[$i]["latitude"],
							'lon' => $sidData[$i]["longitude"]
						)
					)
				);
			}
			else {
				$sid[$sidData[$i]["sidname"]]["waypoints"][] = array(
					'wypname'=> $sidData[$i]["wypname"],
					'lat' => $sidData[$i]["latitude"],
					'lon' => $sidData[$i]["longitude"]
				);
		...
		// Loops through the Query, filters out the correct Procedures.
 		for($i=0; count($starData) > $i; $i++) {
			if (empty($star[$starData[$i]["starname"]])){
				$star[$starData[$i]["starname"]] = array(
					'icao' => $starData[$i]["icao"],
					'aerodome' => array(
							'lat' => $coordinates[0]["lon"],
							'lon' => $coordinates[0]["lat"]
					),
					'runways'=> $starData[$i]["runways"],
					'waypoints' => array(
						array(
							'wypname'=> $starData[$i]["wypname"],
							'lat' => $starData[$i]["latitude"],
							'lon' => $starData[$i]["longitude"],
							'altitude' => $starData[$i]["altitude"]
						)
					)
				);
			}
			else {
				$star[$starData[$i]["starname"]]["waypoints"][] = array(
					'wypname'=> $starData[$i]["wypname"],
					'lat' => $starData[$i]["latitude"],
					'lon' => $starData[$i]["longitude"],
					'altitude' => $starData[$i]["altitude"]
			...
			if(is_numeric(substr($starApp[$i]["name"], -1))){
				if(substr($starApp[$i]["name"], -2) == $runway){
					...
					if($starApp[$i]["index"] == 0){
						$appr[$starApp[$i]["name"]] = array(
							'icao' => $starApp[$i]["icao"],
							'waypoints' => array(
								array(
									'lat' => $starApp[$i]["latitude"],
									'lon' => $starApp[$i]["longitude"],
									'type' => $starApp[$i]["type"]
								)
							)
						);
					} else {
						$appr[$starApp[$i]["name"]]["waypoints"][] = array(
							'lat' => $starApp[$i]["latitude"],
							'lon' => $starApp[$i]["longitude"],
							'type' => $starApp[$i]["type"]
						);
