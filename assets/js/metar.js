...
document.getElementById("metar-form").addEventListener("submit", function (e) {
    e.preventDefault();
    decode();
});
...
function decode() {
    ...
    let icaoInput = document.getElementById("icao").value;
    let icao = icaoInput.toUpperCase();

    if (icao == "" || icao.length !== 4) {
        s.innerHTML = "Wrong ICAO!";
        s.className = "show";
        setTimeout(function () {
    ...
    Info.open("GET", "https://atccom.de/api/metar/" + icao, true);

    Info.onreadystatechange = function () {
        if (Info.readyState != 4) return;

        let usableInfo = JSON.parse(Info.responseText);
        ...
        if (usableInfo.error) {
            ...
            metar_decode(usableInfo.weather.metar);
        }
    }
    Info.send();
}
...
if (str) {
    ...
    if (objURL.icao == "") {
        ...
        document.getElementById("icao").value = objURL.icao;
        decode();
    }
...
function decode_token(token) {
    ...
    if (token == "WS") {
        add_output("There is wind-shear in ");
        return;
    } else if (token == "ALL") {
        add_output("all ");
        return;
